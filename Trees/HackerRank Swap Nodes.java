...
For this in-order traversal, start from the left child of the root node and keep exploring the left subtree until you reach a leaf. When you reach a leaf, back up to its parent, check for a right child and visit it if there is one.
If there is not a child, you've explored its left and right subtrees fully. If there is a right child, traverse its left subtree then its right in the same manner. Keep doing this until you have traversed the entire tree. You will
...
public class SwapNodes {
    public static int[][] indexes = new int[1024][2];
    ...
    public static void inorder(BinaryTree head) {
        if (head == null) return;
        if (head.getLeft().getValue() != -1) inorder(head.left);
        System.out.print(head.getValue() + " ");
        if (head.getRight().getValue() != -1) inorder(head.right);
    }

    public static void swapTree(BinaryTree node, int query) {
        if (node == null) return;
        int depth = 1;
        Deque<BinaryTree> q = new LinkedList<>();
        q.add(node);

        while (!q.isEmpty()) {
            int size = q.size();
            //// for every level (level % query == 0) of the tree swap children
            while (size-- > 0) {
                BinaryTree curr = q.peekFirst();
                if (depth % query == 0) {
                    swap(curr);
                }
                if (curr.getLeft() != null) q.addLast(curr.getLeft());
                if (curr.getRight() != null) q.addLast(curr.getRight());
                q.removeFirst();
            }
            depth++;
        }
    ...
    public static void fillTree(BinaryTree head) {
        if (head == null) return;

        BinaryTree leftNode = new BinaryTree(indexes[head.getValue() - 1][0]);
        BinaryTree rightNode = new BinaryTree(indexes[head.getValue() - 1][1]);

        head.setLeft(leftNode);
        head.setRight(rightNode);
        if (leftNode.getValue() != -1) fillTree(leftNode);
        if (rightNode.getValue() != -1) fillTree(rightNode);
    }
}
